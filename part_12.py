...
from maps_ip import get_map_image, object_search, object_adress, get_object_postal_code, search_organization
...
class MainWindow(QMainWindow):
    ...
    def search_f(self, f=False):
        ...
        if new_coords is not None:
            ...
            if not self.postal_code_btn.isChecked():
                ...
                self.adress_text.setText(object_adress(name))
            if new_coords not in self.metka_coords:
                self.metka_coords.append(new_coords)
                self.metka += [
    ...
    def mousePressEvent(self, event):
        if (event.button() == Qt.LeftButton):
            ...
            a, b = self.map_size
            px, py = 500 // 2, 450 // 2
            self.search.setText(str(self.map_center[0] + (a / px) * (
                x - px)) + "," + str(self.map_center[1] - (b / py) * (y - py)))
            self.search_f(f=True)
        if (event.button() == Qt.RightButton):
            x, y = event.x(), event.y()
            a, b = self.map_size
            px, py = 500 // 2, 450 // 2
            coords = str(self.map_center[0] + (a / px) * (
                x - px)) + "," + str(self.map_center[1] - (b / py) * (y - py))
            zap = search_organization(coords)
            if zap is not None:
                new_coords = zap[1]
                if new_coords not in self.metka_coords:
                    self.metka_coords.append(new_coords)
                    self.metka += [
                        f"{new_coords[0]},{new_coords[1]},pmywm{len(self.metka) + 1}"]
                    self.search.setText("")
                    self.adress_text.setText(zap[0])
                    self.update_map_image()
